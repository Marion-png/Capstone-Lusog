...
class AdviserDashboardOverviewTest extends TestCase
{
    ...
    #[Test]
    public function topbar_search_dropdown_embeds_the_advisers_own_roster(): void
    {
        $response = $this->withSession($this->adviserSession())->get('/dashboard/class-adviser');
    ...
    #[Test]
    public function a_success_flash_renders_the_shared_toast(): void
    {
        $response = $this->withSession($this->adviserSession())
            ->withSession(['success' => 'Deworming request accepted successfully.'])
            ->get('/dashboard/class-adviser');

        $response->assertStatus(200);
        $response->assertSee('id="asbToast"', false);
        $response->assertSee('Deworming request accepted successfully.');
    }

    #[Test]
    public function needs_attention_panel_shows_a_positive_footer_when_some_students_are_clear(): void
    {
        StudentHealthRecord::create([
            'institution_id' => $this->inst->id,
            'school_year' => StudentHealthRecord::currentSchoolYear(),
        ...
        $joseRecord = StudentHealthRecord::where('student_id', 'LRN001')->first();
        ...
        HealthAssessment::create([
            'student_health_record_id' => $joseRecord->id,
            'school_year' => HealthAssessment::currentSchoolYear(),
            'submitted_by_name' => 'Maria Santos',
        ]);

        // Sofia has no consent form and no assessment -> she gets flagged too,
        // so with 2/2 students flagged there should be NO "clear" footer note.
        $response = $this->withSession($this->adviserSession())->get('/dashboard/class-adviser');
        $response->assertDontSee('no pending issues');

        // Give Sofia a completed assessment so she's the only clear student.
        StudentHealthRecord::create([
            'institution_id' => $this->inst->id,
            'school_year' => StudentHealthRecord::currentSchoolYear(),
            'student_id' => 'LRN002',
        ...
        $response = $this->withSession($this->adviserSession())->get('/dashboard/class-adviser');
        $response->assertSee('1 other student has no pending issues');
    }
}
