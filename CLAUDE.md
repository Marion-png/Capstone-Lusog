...
4. **The dashboard is live and derived** (`FeedingCoordinatorController::dashboard` → `feed-dashboard.blade.php`). The SBFP header counts the feeding day from `App\Support\FeedingProgramCycle` (day 1 = first recorded session, shared with the Feeding Program page). Below it sit four cards (Beneficiary / Attendance / At-Risk / Awaiting Enrollment), an **Attendance Monitoring** panel for today's session and a **Nutritional Status** roll of beneficiaries by baseline status. All three panels are `live-pane`s: the page polls `dashboard.feedingcor.metrics.pulse` (a stamp, no PII — exempt in `AuditSensitiveAccess`) every 20s and re-renders from `dashboard.feedingcor.metrics`, which returns the **same Blade partials** the first paint used (`feedingcor-dashboard/partials/*`), so live and reloaded views cannot drift. Today's panel keeps four states — present / absent / unconfirmed / not recorded — and a day with no marks reports "not recorded", not 0%. `FeedingCoordinatorDashboardTest` guards this.
